...
namespace ns3 {
    ...
    Txop::NotifyAccessGranted (void)
    {
        ...
        if (m_currentPacket == 0)
        {
            ...
            NS_ASSERT (item != 0);
            NS_ASSERT (!m_replaceBeacon.IsNull ());
            if (item->GetHeader ().GetType () == WIFI_MAC_MGT_NULL_BEACON)
            {
    ...
    void
    Txop::SetRepalceBeaconCallback (Callback <Ptr<WifiMacQueueItem>, Ptr<WifiMacQueueItem>> callback)
    {
