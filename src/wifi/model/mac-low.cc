...
/* static */
    ...
    void
    MacLow::SetEndTxNoAckCallback (Callback <void, WifiMacHeader &> callback)
    {
        m_endTxNoAckCallback = callback;
    }
    ...
    MacLow::EndTxNoAck (void)
    {
        ...
        if (!m_endTxNoAckCallback.IsNull ())
        {
            m_endTxNoAckCallback (m_currentPacket->GetHeader (0));
        }
        else
        {
            NS_FATAL_ERROR("MacLow - Callback is null");
        }
        if (!IsCfPeriod ())
        {
            m_currentTxop = 0;
